...
public class ComparatorExample {
    //tabelul FLRS pentru T0 care implementeaza P0-P1- se face scaderea elem din tabel (1,1)(adica:NL-NL),(1,2)..(5,5)
    ...
    //tabelul FLRS pentru T1 care face selectia in functie de rezultatul P0-P1 (pozitiv sau negativ)    // lab 2 prima pagina sunt cele 3 relatii cu epsilo doar ca p3 si p4
//    String separator="{[<NL,FF><NL,FF><FF,FF><FF,PL><FF,PL>]}";->asa era in lab dar e inversat P5 cu P6
...
// se creeaza executorul pentru reteaua Petri data si se specifica perioada in milisecunde
        AsyncronRunnableExecutor executor = new AsyncronRunnableExecutor(petriNet, 20); //se specifica reteaua petri si perioada(durata unui tic)

//  se creeaza un obiect pentru vizualizarea comportamentului retelei Petri
        FullRecorder recorder = new FullRecorder(); //clasa recorder memoreaza starea retelei
...
// construim colectia de tip dictionar (map) pentru intrari
            ...
            if (i % 10 < 5) { //conditia da iesirile, daca rez i %10 <5 atunci P3>P4 =>t2 altfel t3
...
// se pun jetoanele de intrare pentru executor
            executor.putTokenInInputPlace(inps); //punem jetoane in locatiile de intrare
            try {
                Thread.sleep(5);
        ...
        executor.stop(); // oprim executorul
