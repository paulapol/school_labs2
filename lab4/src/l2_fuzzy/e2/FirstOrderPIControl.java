...
public class FirstOrderPIControl {
    ...
    String doubleChannelAdder = ""// (x1+x2)
    ...
    String doubleChannelDifferentiator = ""// (x2-x1)
            + "{[<ZR,ZR><PM,PM><PL,PL><PL,PL><PL,PL>]" //
            + " [<NM,NM><ZR,ZR><PM,PM><PL,PL><PL,PL>]" //
            + " [<NL,NL><NM,NM><ZR,ZR><PM,PM><PL,PL>]"//
            + " [<NL,NL><NL,NL><NM,NM><ZR,ZR><PM,PM>]"//
    ...
    String doubleChannelDifferentiator2 = ""// (x1-x2)
...
// se construieste reteaua Petri
        ...
        int p0 = net.addPlace();
        net.setInitialMarkingForPlace(p0, FuzzyToken.zeroToken()); //adauga jeton in locatie
        ...
        int p2InpSys = net.addInputPlace(); //locatie de intrare
        ...
        int p4InpCmd = net.addInputPlace(); //locatie de intrare
        ...
        net.setInitialMarkingForPlace(p7Mem, FuzzyToken.zeroToken()); //adauga jeton in locatie
        ...
        net.setInitialMarkingForPlace(p11Mem, FuzzyToken.zeroToken()); //adauga jeton in locatie
        net.addArcFromTransitionToPlace(t6delay, p11Mem);
        net.addArcFromPlaceToTransition(p11Mem, t5, 1.0);

        int t7Out = net.addOuputTransition(OneXOneTable.defaultTable()); //tranzitie de iesire cu actiune
        ...
        net.addActionForOuputTransition(t7Out, new Consumer<FuzzyToken>() {
                    ...
                    public void accept(FuzzyToken t) {
                        System.out.println("Output From Transition 7: " + t.shortString());
